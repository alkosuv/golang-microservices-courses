// Паттерн Barrier в Go позволяет синхронизировать несколько горутин, чтобы они могли продолжить выполнение только после того,
// как все они достигнут определенной точки. Это может быть полезно в ситуациях, когда требуется, чтобы все горутины завершили
// определенные операции перед тем, как продолжить.
package main
